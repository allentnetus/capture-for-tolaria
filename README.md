...
当前交付版本是 `v0.1.0-beta.3`：Windows + Chrome + Article Capture + Direct File Channel。用户在公开文章页面点击 Capture，Extension 提取并清理正文，Native Messaging Helper 将普通 Markdown 和成功本地化的图片原子地写入用户授权的 Tolaria Vault。

当前源码基线：`v0.1.0-beta.3`。`v0.1.0-alpha.1`、`v0.1.0-beta.1` 和 `v0.1.0-beta.2` 是已公开的历史预览版；Beta.3 作为当前 GitHub Pre-release，尚未提交 Chrome Web Store。

当前 Beta.3 能力：对公开 Article 正文中识别到的图片执行本地化，将成功下载的图片以 `Assets/<sha256>.<ext>` 保存到文章目录；下载失败时保留远程引用并显示回退摘要。

当前 Beta.3 还提供 Extension 的 `Settings` 页面，可自定义 Vault 根目录和 Vault 内默认目录。默认目录仍为 `Inbox/Web`；设置页面的本地实现和代码级门禁已完成，真实 Chrome/Tolaria 用户链路仍需单独验收。
...
Beta.3 暂不实现 MCP 9710、AI、多 Vault、Selection、Bookmark、Screenshot、Edge、macOS、Linux、云同步或账号系统。

## 安装 Beta.3

1. 从 [GitHub Release `v0.1.0-beta.3`](https://github.com/allentnetus/capture-for-tolaria/releases/tag/v0.1.0-beta.3) 下载唯一的用户安装包：`capture-for-tolaria-installer-v0.1.0-beta.3.zip`。不需要另行下载 Extension ZIP、Helper EXE、源码或开发依赖。
...
powershell -NoProfile -ExecutionPolicy Bypass -File installer/windows/assemble-release.ps1 -ExtensionDirectory .\.beta-extension-dist -ReleaseTag v0.1.0-beta.3
```
...
如果当前网络把公共域名解析到 fake-IP 地址，且确认该映射由本机可信网络代理提供，可以在配置 Vault 时显式启用 Beta.3 兼容模式：
