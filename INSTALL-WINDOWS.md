...
`v0.1.0-beta.4` 安装到当前用户目录，不需要管理员权限，不写入 `Program Files`，不注册系统级 Native Host。目标用户不需要安装 Node.js。
...
- `v0.1.0-beta.4` 的单个 Installer ZIP：`capture-for-tolaria-installer-v0.1.0-beta.4.zip`
- 解压后的 Installer ZIP 根目录应同时存在 `VERSION`、`extension`、`installer` 和 `capture-for-tolaria-helper-0.1.0-beta.4-windows-x64.exe`
...
`v0.1.0-beta.4` 构建未签名，Installer ZIP 中的 `extension` 通过 Chrome 开发者模式加载。当前支持 Windows + Chrome + MV3 Article Capture + Direct File Channel，以及正文图片的受限本地化；仍不支持 MCP、AI、Selection、Bookmark、Screenshot、Edge、macOS 或 Linux。不要把私钥、用户配置或 Vault 数据放进 ZIP。
