...
- 图片字节不会进入 Native Messaging payload，也不会上传云端；CSS 背景图、`blob:`、登录后图片和需要防盗链凭据的资源不属于 Beta.2 范围。如果用户显式为当前本机 fake-IP 网络启用 `allowSyntheticDns`，Helper 仍不读取或发送 cookies、`Authorization` 或页面凭据；该开关只允许当前代理映射段继续使用无凭据请求，默认关闭。
